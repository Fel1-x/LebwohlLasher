...
@cython.boundscheck(False)
@cython.wraparound(False)
def all_energy(double[:,::1] arr, int nmax, int threads):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_order(double[:,::1] arr, int nmax, int threads):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
    ...
    """
    Qab = np.zeros((3,3))
    cdef double[:,:,::1] temp_Qab = np.zeros((nmax, 3,3), dtype=np.double)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def MC_step(double[:,::1] arr, float Ts, int nmax, int threads):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
	  Ts (float) = reduced temperature (range 0 to 2);
...
def main(program, int nsteps, int nmax, float temp, int pflag, int num_threads):
    ...
    energy[0] = all_energy(lattice,nmax,threads=num_threads)
    ratio[0] = 0.5 # ideal value
    order[0] = get_order(lattice,nmax, threads=num_threads)
    ...
    for it in range(1,nsteps+1):
        ...
        energy[it] = all_energy(lattice,nmax,threads=num_threads)
        order[it] = get_order(lattice,nmax, threads=num_threads)
